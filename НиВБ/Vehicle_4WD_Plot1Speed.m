...
if ~exist('simlog_Vehicle_4WD_NiVB', 'var')
    sim('Vehicle_4WD_NiVB')
end

% Использовать существующее окно графика, если оно уже есть, иначе создать новое окно
if ~exist('h1_Vehicle_4WD_NiVB', 'var') || ...
        ~isgraphics(h1_Vehicle_4WD_NiVB, 'figure')
    h1_Vehicle_4WD_NiVB = figure('Name', 'Vehicle_4WD_NiVB');
end
figure(h1_Vehicle_4WD_NiVB)
clf(h1_Vehicle_4WD_NiVB)

% Получить результаты моделирования
simlog_t = simlog_Vehicle_4WD_NiVB.Vehicle_Body.V.series.time;
simlog_vehSpd = simlog_Vehicle_4WD_NiVB.Vehicle_Body.V.series.values('km/hr');
simlog_wEngine = simlog_Vehicle_4WD_NiVB.Engine.F.w.series.values('rpm');
simlog_thrEngine = simlog_Vehicle_4WD_NiVB.Engine.ThrNorm.series.values;
